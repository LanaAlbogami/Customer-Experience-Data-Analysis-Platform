...
def fetch_records_from_db():
    ...
    بكثير لما يكبر عدد السجلات. نفس الأسلوب المستخدم بالأفراد.
    """
...
def prepare_uploaded_records(
    ...
    if not entity_column:
        raise ValueError(
            "يجب اختيار عمود اسم الجهة الحكومية."
        )

    if entity_column not in available_columns:
        raise ValueError(
            f"عمود الجهة غير موجود في الملف: {entity_column}"
        )
...
# ==================================================
# دالة مستقلة لصفحة "تحليل الجهات" بس (Departments/entities_analysis_page.py)
# نسخة منفصلة تمامًا عن fetch_records_from_db()، ما تؤثر عليها أبدًا.
# ==================================================

@st.cache_data(ttl=300, show_spinner="جاري تحميل بيانات الجهات...")
def fetch_records_with_entity():
    """
    نسخة مستقلة من fetch_records_from_db() تجيب اسم الجهة الحكومية
    كمان (عن طريق ربط إضافي مع جدول GovernmentEntities).

    مخصصة بس لصفحة "تحليل الجهات"، وما تؤثر على أي صفحة ثانية
    تستخدم fetch_records_from_db() العادية.
    """
