...
#st.set_page_config(page_title="التقارير - منصة تجربة العميل", layout="wide", initial_sidebar_state="collapsed")
    ...
    try:
        ...
        with connection.cursor() as cursor:
            for period_str in selected_periods:
                ...
                query = """
                    SELECT d.DepartmentName, s.ServiceName, res.PrevValue, res.TargetValue, res.CurrentValue, ind.IndicatorName, mr.Year, mr.Period
...
# كود جافاسكريبت خفيف ونظيف لإزالة زر Select all ومساحته تماماً من القائمة المنسدلة
components.html(
    """
    <script>
    const cleanSelectAll = () => {
        const doc = window.parent.document;
        const items = doc.querySelectorAll('div[data-baseweb="popover"] li, div[data-baseweb="popover"] div[role="option"]');
        items.forEach(item => {
            if (item.innerText.includes("Select all") || item.innerText.includes("Select All")) {
                let parentDiv = item.closest('li') || item;
    ...
    const observer = new MutationObserver(cleanSelectAll);
...
if submit_button:
    if not selected_periods or selected_format is None:
        st.error("⚠️ الرجاء اختيار فترة واحدة على الأقل وتحديد صيغة الملف.")
    else:
...
# هذا الكود يشتغل عشان الداتابيس الجديدة لان الاسماء تغيرت
            '''import streamlit as st
import pymysql
import os
from dotenv import load_dotenv
